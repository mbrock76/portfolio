...
function init(){
	...
	document.getElementById("code").addEventListener("click", loadCode);
	...
	for(let i = 0; i < tempCodes.length; i++){
		document.getElementById(tempCodes[i].id).addEventListener("click", setCodeNavFocus);
	}
	...
	document.getElementById("mrp-user-logout").addEventListener("click", logout);
	document.getElementById("mrp-user-remember").addEventListener("click", rememberMe);
	
	//mrp collapse/expand arrow toggles
	document.getElementById("mrp-rou-CE").addEventListener("click", mrpToggleContent);
	document.getElementById("mrp-inv-CE").addEventListener("click", mrpToggleContent);
	document.getElementById("mrp-rec-CE").addEventListener("click", mrpToggleContent);
	document.getElementById("mrp-shi-CE").addEventListener("click", mrpToggleContent);
	document.getElementById("mrp-por-CE").addEventListener("click", mrpToggleContent);
	document.getElementById("mrp-sor-CE").addEventListener("click", mrpToggleContent);
	document.getElementById("mrp-doc-CE").addEventListener("click", mrpToggleContent);
	...
	document.getElementById("games-user-remember").addEventListener("click", rememberMe);
	...
	function setNavPos(){
		...
		let navheight = document.getElementById("title").getBoundingClientRect().bottom;
	...
	function hideChildren(myId){
		let children = document.getElementById(myId).children;
	...
	function setNavBtnFocus(myId){

		hideChildren("center");
		let children = document.getElementById("nav").children;

		for(let i = 0; i < 4; i++)
		{
			if(document.getElementById(children[i].id).classList.contains("navBtnFocus")){
				
				document.getElementById(children[i].id).classList.remove("navBtnFocus");
				document.getElementById(children[i].id).classList.add("navBtn");
			}

		}

		document.getElementById(myId).classList.add("navBtnFocus");
		document.getElementById(myId).classList.remove("navBtn");
		document.getElementById(myId + "-content").style.display = "inline-block";
	...
	function valUsername(){
		...
		let temp = document.getElementById("mrp-login-username").value.replace(/[^a-z0-9_]/gi, "");
	...
	function valPassword(){
		...
		let temp = document.getElementById("mrp-login-password").value.replace(/[^a-z0-9_]/gi, "");
	...
	function login(e){
		...
		let prefixId = tempId[0];
		
		user.name = document.getElementById(`${prefixId}-login-username`).value;
		user.pwd = document.getElementById(`${prefixId}-login-password`).value;
		user.date = Date.now();
		...
		if(!user.name){
			...
			checkUser(setUser, user, prefixId);
		}
	}
	...
	function logout(e){
		...
		user = {name: "", pwd: "", date: 0, remember: false};
		hideChildren(`${prefixId}-content`);
		
		document.getElementById(`${prefixId}-user-remember`).checked = user.remember;
		document.getElementById(`${prefixId}-welcome`).style.display = "block";
		document.getElementById(`${prefixId}-login`).style.display = "inline-block";
	}
	
	//saves or clears the user name in local storage based on a checkbox
	function rememberMe(e){
		
		let tempId = e.target.id.split("-");
		let prefixId = tempId[0];
		
		if(document.getElementById(`${prefixId}-user-remember`).checked){
			user.remember = true;
	...
	function loadAbout(e){
		...
		if(document.getElementById(e.target.id).classList.contains("navBtn")){
			setNavBtnFocus(e.target.id);
			
			if(mobile){
				document.getElementById("center").style.margin = "0 5% 0 5%";
			}
		}
	}
	
	//loads the code blocks with pre-highlighted code
	function loadCode(e){
		
		let myCodeSets = ["setCodeHtml", "setCodeCss", "setCodeJs", "setCodePhp", "setCodePy"];
		let myCodeScripts = ["scripts/get_html.php", "scripts/get_css.php", "scripts/get_js.php", "scripts/get_php.php", "scripts/get_py.php"];
		
		for(let i = 0; i < myCodeSets.length; i++){
			get(eval(myCodeSets[i]), myCodeScripts[i]);
		}
	...
	function setCodeNavFocus(e){
		
		hideChildren("code-content-block");
		let codeChildren = document.getElementById("code-content-nav").children;
		let temp = e.target.id.split("-");
		let mySuffix = temp[temp.length-1];
		
		for(let i = 0; i < codeChildren.length; i++){
				document.getElementById(codeChildren[i].id).className = "code-content-nav-tabs";
		}
		
		if(document.getElementById(e.target.id).className == "code-content-nav-tabs"){
			
			document.getElementById(e.target.id).className = "code-content-nav-focus";
		}
		
		document.getElementById(`code-content-block-${mySuffix}`).style.display = "block";
	}
	
	//initial loading for the marp section, checks for username
	function loadMrp(e){
		...
		if(document.getElementById(e.target.id).classList.contains("navBtn")){
			...
			hideChildren("mrp-content");
			
			if(temp != null){

				user = JSON.parse(temp);
				document.getElementById("mrp-user-name").innerHTML = user.name;
				document.getElementById("mrp-user-remember").checked = user.remember;
				document.getElementById("mrp-wrapper").style.display = "inline-block";
				...
				document.getElementById("mrp-welcome").style.display = "block";
				document.getElementById("mrp-login").style.display = "inline-block";
			}
	...
	function mrpToggleContent(e){
		
		let myVal = document.getElementById(e.target.id).innerHTML;
		if(myVal == "▲"){
			document.getElementById(e.target.parentNode.parentNode.id + "-content").style.display = "block";
			document.getElementById(e.target.id).innerHTML = "▼";
		}
		else{
			document.getElementById(e.target.parentNode.parentNode.id + "-content").style.display = "none";
			document.getElementById(e.target.id).innerHTML = "▲";
		}
	...
	function loadRou(){
		...
		myString += span;
		if(mobile){
			myString += `<table class="table-data"><tr><th></th><th>ID:</th><th>Dept:</th><th>Qty:</th></tr>`;
		}else{
			myString += `<table class="table-data"><tr><th></th><th>ID:</th><th>Dept:</th><th id="mrp-rou-table-th">Description:</th><th>Qty:</th></tr>`;
		}
		...
		for(let i = 0; i < sors.length; i++){
			...
			for(let j = 0; j < invs.length; j++){
				if(parseInt(invs[j].qty) > 0 && invs[j].loc != "REC"){
					switch(invs[j].partID){
						case "1":
							locs[1].push(invs[j].loc);
							break;
						case "2":
							locs[2].push(invs[j].loc);
							break;
						case "3":
							locs[3].push(invs[j].loc);
							break;
						case "4":
							locs[4].push(invs[j].loc);
							break;
						case "5":
							locs[5].push(invs[j].loc);
							break;
					}
				}
			}
			...
			myString += `${rowTag}<td><input type="checkbox" class="rous" id="rou${sors[i].id}" value="${sors[i].id}"></td>`;
			...
			if(mobile){
				...
				myString += `${rowTag}<td colspan="4">${partDesc[partNames[sors[i].partID]][sors[i].step]}</td>`;
			}else{
	...
	function loadInvs(){
		...
		myString += span;
		myScraps += span;
		myString += `<table class="table-data"><tr><th></th><th>Loc:</th><th>partID:</th><th>desc:</th><th>qty:</th></tr>`;
		myScraps += `<table class="table-data"><tr><th>PartID:</th><th>Desc:</th><th>Inv:</th><th>Pors:</th><th>Sors:</th><th>Scrap:</th></tr>`;
		...
		for(let i = 0; i < invs.length; i++){
			...
			myString += `<td><input type="checkbox" class="invs" id="inv${invs[i].id}" value="${i}"></td>`;
	...
	function loadRecs(){
		...
		myString += span;
		myString += `<table class="table-data"><tr><th></th><th>order#</th><th>partID</th><th>desc.</th><th>qty.</th><th>order date</th><th>del. date</th></tr>`;
	...
	function loadShips(){
		...
		myString += span;
		myString += `<table class="table-data"><tr><th></th><th>ID</th><th>Desc</th><th>Qty</th><th>Bol</th><th>Carrier</th><th>Date</th></tr>`;
	...
	function loadPors(){
		...
		myString += span;
		myString += `<table class="table-data"><tr><th></th><th>order#</th><th>desc.</th><th>qty.</th><th>emp.</th><th>order date</th><th>del. date</th></tr>`;
	...
	function loadSors(){
		...
		myString += span;
		myString += `<table class="table-data"><tr><th></th><th>ID</th><th>Desc</th><th>Qty</th><th>Status</th><th>Emp</th><th>Date</th></tr>`
	...
	function loadGames(e){
		...
		if(document.getElementById(e.target.id).classList.contains("navBtn")){
			...
			if(mobile){
				document.getElementById("center").style.margin = "0 1% 0 1%";
			...
			if(temp != null){
				...
				document.getElementById("games-user-remember").checked = user.remember;
				...
				document.getElementById("games-welcome").style.display = "block";
	...
	function switchGame(myId){
		...
		for(let i = 0; i < gameNames.length; i++){
			document.getElementById(`games-nav-buttons-${gameNames[i]}`).style.outline = "2px solid transparent";
		}
	...
	function tttClick(e){
		...
		if(e.clientX){
			...
			let myX = Math.floor((e.clientX - tttRect.left)/ tttSize);
			let myY = Math.floor((e.clientY - tttRect.top)/ tttSize);
		}else if(e.touches[0].clientX){
			
			let myX = Math.floor((e.touches[0].clientX - tttRect.left)/ tttSize);
			let myY = Math.floor((e.touches[0].clientY - tttRect.top)/ tttSize);
		}else{
	...
	function checkUser(myCallback, myObj, myPrefixId){
		...
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				myCallback(this.responseText, myPrefixId);
			}
		};
	...
	function setUser(myData, prefixId){

		if(myData == "wrong password"){
			
			alert("Incorrect user/password combination.");
			
		}else if(myData == "not found"){
			
			post(user, scriptNames.post_user);
			alert("Welcome " + user.name + "!");
			hideChildren(`${prefixId}-content`);
			localStorage.setItem("user", JSON.stringify(user));
			document.getElementById(`${prefixId}-user-name`).innerHTML = user.name;
			document.getElementById(`${prefixId}-wrapper`).style.display = "inline-block";
			setUserScores();
	...
	//set the code blocks with pre-highlighted code from text files
	function setCodeHtml(myData){
		
		document.getElementById("code-content-block-html").innerHTML = myData;
	}
	
	function setCodeCss(myData){
		
		document.getElementById("code-content-block-css").innerHTML = myData;
	}
	
	function setCodeJs(myData){
		
		document.getElementById("code-content-block-js").innerHTML = myData;
	}
	
	function setCodePhp(myData){
		
		document.getElementById("code-content-block-php").innerHTML = myData;
	}
	
	function setCodePy(myData){
		
		document.getElementById("code-content-block-py").innerHTML = myData;
	}
	...
	function getUsers(){
		...
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				lines = JSON.parse('[' + this.responseText.replace(/}{/g, '},{') + ']');
			}
		};
